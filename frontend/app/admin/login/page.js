...
function AdminLoginForm() {
  ...
  const deactivated  = searchParams.get('reason') === 'subscription_deactivated'
  ...
  return (
    <div className="bg-white rounded-2xl shadow-lg border border-slate-200 p-8">
      ...
      <div className="text-center mb-8">
        ...
        <h1 className="text-2xl font-bold text-slate-800">School Login</h1>
        <p className="text-slate-500 text-sm mt-1">Sign in to your school dashboard</p>
      </div>

      {/* Subscription deactivated banner */}
      {deactivated && (
        <div className="mb-4 px-4 py-3 bg-amber-50 border border-amber-300 rounded-lg text-amber-800 text-sm">
          <p className="font-semibold">Your school subscription has been deactivated.</p>
          <p className="mt-0.5">Please contact support to reactivate your account.</p>
        </div>
      )}
      ...
      <form action={action} className="space-y-5">
        <div>
          ...
          <input
            ...
            required
            autoComplete="email"
            className="w-full px-4 py-2.5 border border-slate-300 rounded-lg text-slate-800 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-transparent transition"
